...
os.chdir(BASE_DIR)  # s'assurer qu'on est dans le bon dossier pour Git
...
with open(CSV_FILE, encoding="utf-8") as f:
    ...
    for row in reader:
        date = row["date"]
        server = row["server"]
        race = RACE_TRANSLATION.get(row["race"], row["race"])
...
for s in ["R1","R2","R4","R3","R7","R14"]:
    html += f"""<label><input type="checkbox" class="server-filter" value="{s}" checked>{SERVER_TRANSLATION.get(s,s)}</label>"""

html += "<br><strong>Races :</strong><br>"
races = sorted(list({p["race"] for p in all_players}))
for r in races:
    html += f"""<label><input type="checkbox" class="race-filter" value="{r}" checked>{r}</label>"""
...
function applyPreset(days){{
    ...
    for(let d of dates_sorted) if(d>=startStr){{closestStart=d; break;}}
...
function updateRaceStats(){{
    ...
    let total = 0;
    rowsData.forEach(row=>{{ raceCounts[row[2]]=(raceCounts[row[2]]||0)+1; total++; }});
    let html = `<strong>Total joueurs affichés :</strong> ${total}<br><br>`;
    for(let race in raceCounts) html+=`${race} : <strong>${raceCounts[race]}</strong><br>`;
...
$(document).ready(function(){{
    ...
    document.querySelectorAll('.server-filter,.race-filter').forEach(el=>el.addEventListener('change',updateProgression));
    ...
    $('#progressTable').on('draw.dt',()=>updateRaceStats());
    updateProgression();
}});
</script>
</body>
